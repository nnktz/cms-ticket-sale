...
import { fetchFamilyTickets } from "../../../modules/ticketManagement/actions";
...
const FamilyPackage = () => {
  ...
  const { error, familyTickets } = useSelector(
  ...
  React.useEffect(() => {
    let count = 1;
    if (familyTickets && !(valueSearch.length > 0)) {
      const newData: IData[] = familyTickets.map((data) => ({
        key: count++,
  ...
  }, [familyTickets, valueSearch.length]);
